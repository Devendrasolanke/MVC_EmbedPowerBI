...
namespace AppOwnsData.Services
{
    ...
    public static class EmbedService
    {
        ...
        //public static async Task<ReportEmbedConfig> GetEmbedParams(int DealerID, int DealerBranchID, string DealerType, Guid workspaceId, Guid reportId, [Optional] Guid additionalDatasetId)
        public static async Task<ReportEmbedConfig> GetEmbedParams(Guid workspaceId, Guid reportId, [Optional] Guid additionalDatasetId)
        {
            ...
            {
                //code for sms changes for 4.0 03 mar 2023
                ...
                {
                    //Sales
                    //pbiReport.EmbedUrl = pbiReport.EmbedUrl + $"&filter=Fact%2FDealerBranchID%20eq%20{DealerBranchID}%20and%20Fact_Commissioning%2FDealerBranchID%20eq%20{DealerBranchID}%20and%20OEM%2FDealerBranchID%20eq%20{DealerBranchID}"; //OLD FILTER LINK

                    //pbiReport.EmbedUrl = pbiReport.EmbedUrl + $"&filter=Dim_DealerID%2FDealerBranchesID%20eq%20{DealerBranchID}"; //NEW FILTER LINK
                    //pbiReport.EmbedUrl = pbiReport.EmbedUrl + $"&filter=Dim_DealerID%2FDealerBranchesID+eq+" + DealerBranchID; //NEW FILTER LINK
                    pbiReport.EmbedUrl = pbiReport.EmbedUrl + $"&filter=Dim_DealerID%2FDealerBranchesID%20eq%20" + DealerID; //NEW FILTER LINK
                ...
                {
                    //Service
                    //pbiReport.EmbedUrl = pbiReport.EmbedUrl + $"&filter=Breakdown%2FDealerID%20eq%20{DealerID}%20and%20Population%2FDealerID%20eq%20{DealerID}%20and%20Infold%2FDealerID%20eq%20{DealerID}"; //OLD FILTER LINK

                    pbiReport.EmbedUrl = pbiReport.EmbedUrl + $"&filter=Dim_DealerID%2FDealerID%20eq%20" + DealerID;  //NEW FILTER LINK
                    //pbiReport.EmbedUrl = pbiReport.EmbedUrl + $"&filter=Dim_DealerID%2FDealerID+eq+";  //NEW FILTER LINK



                }*/

                pbiReport.EmbedUrl = pbiReport.EmbedUrl;

                //console.log(pbiReport.EmbedUrl);
        ...
        public static async Task<ReportEmbedConfig> GetEmbedParams1(Guid workspaceId, IList<Guid> reportIds, [Optional] IList<Guid> additionalDatasetIds)
        {
            // Note: This method is an example and is not consumed in this sample app
            //string getpbireport = string.Empty;
            ...
            using (var pbiClient = await GetPowerBiClient())
            {
                ...
                foreach (var reportId in reportIds)
                {
                    ...
                    string filterOption = "&filter=Breakdown%2FDealerID%20eq%20" + /*common.DealerID*/10 + "%20and%20Population%2FDealerID%20eq%20" + /*common.DealerID*/ 10 + "%20and%20Infold%2FDealerID%20eq%20" + /*common.DealerID*/10 + "";

                    var pbiReport = pbiClient.Reports.GetReportInGroup(workspaceId, reportId);
                    pbiReport.EmbedUrl = "https://app.powerbi.com/reportEmbed?reportId=452c8b6e-e2a5-41e1-982b-da2ea6ec6151&autoAuth=true&ctid=8c4858b5-f020-483a-b7ef-71ded6e81767&filter=Fact%2FDealerBranchID%20eq%20447%20and%20Fact_Commissioning%2FDealerBranchID%20eq%20447%20and%20OEM%2FDealerBranchID%20eq%20447";
                    //int result = await client.GetPowerPiAsync(pbiReport, filterOption);
